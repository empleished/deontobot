...
Deontic logic is thought to have first emerged in the fourteenth century, but it was not formalised until the twentieth century when there was a huge shift towards formalising symbolic logic more generally. 

\subsection{Types of Deontic Logic}
%I think SDL And DDL are the most relevant, though I could also include non-monotonic and dynamic?
...
Standard deontic logic faces the contrary-to-duties problem. It's not implausible that it could be forbidden that C, but in the case of C, it could still be expected that A. The classic example is the gentle murderer; murder is forbidden, but if an agent is going to commit murder, they are obliged to do it gently. Standard deontic logic could represent these cases as follows: O¬(C) & C -> O(A), but this is paradoxical and not a satisfactory representation of contrary-to-duties obligations. 

\subsubsection{Dyadic Deontic}
