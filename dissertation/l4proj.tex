...
R-in states that a maxim, m\( \textsubscript{i} \) is allowed to be part of the set of acceptable maxims, M, if m\( \textsubscript{i} \) is consistent with the ethical system, G. That is, m\( \textsubscript{i}  \in M \iff G \vdash m \textsubscript{i} \). For example, %example.

R-out states that if a maxim, m\( \textsubscript{i} \), becomes inconsistent with the ethical system, G, m\( \textsubscript{i} \) is no longer allowed in the set of acceptable maxims, M. That is, m\( \textsubscript{i}  \notin M \rightarrow \neg(G \vdash m \textsubscript{i}) \). For example, %example. 
